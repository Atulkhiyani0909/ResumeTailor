...
export const ResumeDocument = ({ fixedResume }) => {
  ...
  return (
    <div id="resume-preview" className="bg-white text-slate-900 p-10 md:p-14 rounded-xl shadow-[0_0_40px_rgba(0,0,0,0.4)] w-full max-w-[816px] mx-auto font-serif relative z-10 h-max">
      ...
      {achievementsList.length > 0 && (
        <div className="mb-6">
          <h2 className="text-[13px] font-bold border-b border-slate-300 mb-3.5 uppercase tracking-widest text-slate-800">Achievements</h2>
          <ul className="list-disc ml-5 space-y-1.5 text-[13px] text-slate-700 leading-relaxed">
            {achievementsList.map((ach, i) => <li key={i} className="pl-1">{ach}</li>)}
          </ul>
        </div>
      )}
